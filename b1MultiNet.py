...
class B1Node(MCTSNode):
    def __init__(self, propnet, data, game, actions=None, model=None, root=False):
...
def do_game(curl, propnet, model, z=1, N=300, saveTo=None):
    cur = curl[0]
    states = []
    ...
    scores = cur.scores
    for s, p, q in states:
        saveTo.add_sample(s, p, average(scores, q, z), game=self.game)
